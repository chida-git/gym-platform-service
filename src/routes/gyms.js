...
router.put('/:id/profile', async (req, res, next) => {
  try {
    ...
    publishSafe(`personal.upsert.${id}`, { event: 'personal.upsert', gym_id: id, opening_hours: updated.opening_hours, name: updated.name, email: updated.email, phone: updated.phone, description: updated.description, web: updated.web, ts }).catch(()=>{})
