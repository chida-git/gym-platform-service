...
router.post('/plans', async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      publishSafe('catalog', `plan.upsert.${input.gym_id}`, {
      ...
      publishSafe('catalog', `price.upsert.${input.gym_id}`, {
...
router.patch('/plans/:id', async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      if (planTouched) {
        publishSafe('catalog', `plan.upsert.${p2.gym_id}`, {
      ...
      if (priceTouched) {
        ...
        publishSafe('catalog', `${key}.${p2.gym_id}`, {
...
router.delete('/plans/:id', async (req, res, next) => {
  try {
    ...
    const ts = new Date().toISOString()
    publishSafe('catalog', `plan.archive.${p.gym_id}`, { event: 'plan.archive', plan_id: id, gym_id: p.gym_id, ts }).catch(()=>{})
    publishSafe('catalog', `price.archive.${p.gym_id}`, { event: 'price.archive', plan_id: id, gym_id: p.gym_id, ts }).catch(()=>{})
  } catch (e) { next(e) }
})
